...
parser.add_argument("--SKIP_SUPPLEMENTARY","-s2", help = "skip supplementary alignments", default = True)
parser.add_argument("--SKIP_SECONDARY","-s1", help = "skip secondary alignments", default = False)
...
if(args.REPEAT_LEN != 0):
   header += 	"	\"Repeat_Number\"	\"Repeat_Position\""
...
try:
    ...
    for line in sam_file.readlines():
        ...
        if (skip_supplementary and (int(flag) & 2048 == 2048)):
...
for location in range(len(depths)):
    ...
    for variant_type in VARIANT_TYPES:
        ...
        if mode == "SNP":
            if(variant_type == "INS" or variant_type == "INDEL"):
                continue
