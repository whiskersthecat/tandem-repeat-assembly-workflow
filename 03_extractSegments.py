...
parser.add_argument("--undetectedSegmentMinimumLength", "-m", help = "minimum length to include undetected segment between two segments (int)", default = 20)
parser.add_argument("--lenientFirstSegment", "-l", help = "if true, always keep first segment if alignment coordinate starts at 1 (even if its small)", default = False)
...
LENIENT_FIRST = bool(args.lenientFirstSegment)
    ...
    for n, segmentinfo in enumerate(current_segments):
        ...
        if n == 0 or n == num_segs - 1:
            if n == num_segs - 1 and segment_len < MIN_TERMINAL_EDGE_LEN:
                ...
                continue
            ### used for hifi reads
            # if n == 0 and (start_coord == 1 or segment_len < MIN_TERMINAL_EDGE_LEN):
